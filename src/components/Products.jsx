  ...
  return (
    <section id="produits" className="py-20 bg-zinc-950/40 relative">
      <div className="max-w-7xl 2xl:max-w-[1720px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Products Grid: Responsive 2 on mobile, 3 on tablets, 4 to 6 on laptops & wide screens */}
            ...
            {filteredProducts.map((product) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-3 sm:p-4 flex-1 flex flex-col justify-between text-start space-y-2.5">
                    <div className="space-y-1 sm:space-y-1.5">
                      <h3 className="text-xs sm:text-sm font-bold text-white group-hover:text-brand-redLight transition-colors line-clamp-2">
                        {title}
                      </h3>
                    ...
                    <div className="pt-2 sm:pt-2.5 border-t border-zinc-800/80 space-y-2">
                      <div className="flex items-baseline justify-between gap-1">
                        ...
                        <div className="flex items-baseline gap-1">
                          {product.oldPrice && Number(product.oldPrice) > Number(product.price) && (
                            <span className="text-[10px] sm:text-[11px] text-zinc-500 line-through font-mono">
                              {Number(product.oldPrice).toLocaleString()}
                            </span>
                          )}
                          <div className="text-xs sm:text-sm lg:text-base font-black text-white font-mono">
                            {product.price ? product.price.toLocaleString() : "Sur devis"}{" "}
                            <span className="text-brand-redLight text-[10px] sm:text-xs font-bold">
                              {t.products.currency}
                            </span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-1.5 pt-0.5">
                        <button
                          ...
                          className="px-1.5 sm:px-2 py-2 rounded-lg bg-brand-red hover:bg-brand-redDark text-white text-[10px] sm:text-xs font-bold transition-all flex items-center justify-center gap-1 shadow-glow-red hover:scale-[1.02] active:scale-[0.98] cursor-pointer"
                        >
                        ...
                        <button
                          ...
                          className="px-1.5 sm:px-2 py-2 rounded-lg bg-zinc-900 hover:bg-zinc-800 text-zinc-200 hover:text-white text-[10px] sm:text-xs font-bold transition-all flex items-center justify-center gap-1 border border-zinc-700 hover:border-zinc-500 active:scale-[0.98] cursor-pointer"
                        >
                          <Calendar className="w-3 h-3 text-brand-red shrink-0" />
                          <span className="truncate">{t.products.bookBtn || (isRtl ? "موعد" : "RdV")}</span>
                        </button>
                      </div>
                    </div>
